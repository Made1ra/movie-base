...
import { InputLabel, Select, MenuItem, Button, SelectChangeEvent, Typography, Box, Chip } from '@mui/material';
...
export default function Ratings() {
    ...
    function handleDelete(filter: string) {
        ...
        const filteredMovies = ratedMovies.filter((movie) => {
            ...
            const genreMatch = !updatedFilters.genres.length || updatedFilters.genres.some((genre) => movieGenres.includes(genre));
            const typeMatch = !updatedFilters.types.length || updatedFilters.types.some((type) => type.toLowerCase() === movieType.toLowerCase());
            const releaseYearMatch = (!updatedFilters.releaseYearFrom || releaseYear >= updatedFilters.releaseYearFrom) &&
                (!updatedFilters.releaseYearTo || releaseYear <= updatedFilters.releaseYearTo);
            const ratingMatch = (!updatedFilters.ratingFrom || imdbRating >= updatedFilters.ratingFrom) &&
                (!updatedFilters.ratingTo || imdbRating <= updatedFilters.ratingTo);
            const votesMatch = (!updatedFilters.votesFrom || imdbVotes >= updatedFilters.votesFrom) &&
