...
export async function authorizeUser() {
  try {
    ...
    const response = await fetch(
      `https://movie-base-backend-production.up.railway.app/?email=${encodeURIComponent(
        userEmail
      )}`,
      {
        method: "GET",
    ...
    if (responseBody.length > 0) {
      ...
      const newUserResponse = await fetch(
        "https://movie-base-backend-production.up.railway.app/",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
export async function getWatchlist(userID: string) {
  try {
    const response = await fetch(
      `https://movie-base-backend-production.up.railway.app/user/${userID}/watchlist`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
export async function getTitleInfo(movieID: string, userID: string) {
  try {
    const response = await fetch(
      `https://movie-base-backend-production.up.railway.app/title/${movieID}?userID=${userID}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
export async function getRatings(userID: string) {
  try {
    const response = await fetch(
      `https://movie-base-backend-production.up.railway.app/user/${userID}/ratings`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
